  ...
  if (order.toLowerCase().includes('вб')) {
      Object.entries(a).forEach((i, c) => {
        if (i[0].includes("B")) {
          ...
          pallets[currentPallet][getPath(a['A' + currentAdress]["w"],"04.07.2025")].push([a['A' + currentAdress]["w"], "-", +i[1]["w"], +i[1]["w"], "-", +getCount(a['A' + currentAdress]["w"]) * +i[1]["w"]])
          if (getPath(a['A' + currentAdress]["w"],"04.07.2025") == 1) {
